...
function setup() {
  ...
  player = new Doodler(width / 2, height / 2, false, 30, color("Maroon"));
  ...
  for (var i = platforms.length - 1; i >= 0; i--) {
    ...
    if (platforms[i].onScreen) {
      ...
      if (random() < 0.9) {
        ...
        platforms.push(new Platform(x, y, 55, color("gray")));
      } else {
...
function generatePlatforms() {
    ...
    for (var i = 0; i < 3; i++) { // attempt 3 new platforms
      ...
      if (noise(y, i) > 0.5) // 50% chance of a new platform
        field.push(new Platform(x, y, 55, color("gray")));
    }
  }
...
function drawScore() {
  ...
  fill(255);
  noStroke();
  var currentPoints = player.maxAltitude + points;
  text(( + currentPoints).toFixed(0), 50, 50);
}
